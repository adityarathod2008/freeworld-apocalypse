...
This master document establishes the authoritative, standardized, 25-implementation-phase technical blueprint for evolving the existing **FreeWorld** engine into a persistent, living-city survival experience following the completed Phase 0 foundation audit.

---

## 🏛️ Architectural Core Principles & Layering
...
```text
        ...
        ┌───────────────────────────────┼───────────────────────────────┐
        │                               │                               │
  WORLD SYSTEMS                   ENTITY SYSTEMS                  STORY SYSTEMS
        │                               │                               │
  ┌─────┼─────┐                   ┌─────┼─────┐                   ┌─────┼─────┐
  │     │     │                   │     │     │                   │     │     │
 City Power Metro                NPC Vehicle Zombie             Story Cinematic
  │     │     │                   │     │     │                   │     │     │
 GIS   Grid Transit            Brain Physics Horde             Chapter Director
  │                               │     │     │                   │     │
 Roads                        Animation │ Infection            Dialogue │
 Buildings                        │     │     │                   │     │
 Terrain                         IK     │ Perception           Decisions│
 POIs                             │     │     │                         │
                              Facial    │   Migration                   │
                                        └─────┴─────────────────────────┘
                                        │
...
### Development Progression Hierarchy
```text
FOUNDATION → SIMULATION → WORLD → TRAFFIC/VEHICLES → HUMANS/NPC BRAIN → INFRASTRUCTURE/METRO → ZOMBIES → OUTBREAK → STORY → CINEMATICS → EMERGENCY/COMBAT → WEATHER/AUDIO → PERSISTENCE/BUCKET LIST → DEBUG/OPTIMIZATION → MASTER VERTICAL SLICE
```
...
## 📋 Standardized 25 Implementation Phases Roadmap
...
### PHASE 8 — Human Character Model Foundation & Skeletons
- **Objective**: Construct realistic human anatomical skeletons, clothing mesh layers, skin/hair materials, and character rigs.
- **Dependencies**:
  - *Depends on*: Phase 2, Phase 3.
  - *Blocks*: Phase 9, Phase 10, Phase 14.
- **Existing Systems Affected**: `src/NPC/NPCModel.js`, `src/Player/PlayerModel.js`.
- **New Files**: `src/NPC/CharacterRig.js`.
- **Modified Files**: `src/NPC/NPCModel.js`, `src/Player/PlayerModel.js`.
- **Data Contracts & Serialization Schema**: Character appearance configuration `{ gender, height, clothingType, skinTone, hairStyle }`.
- **Runtime Behavior**: Assembles 3D character mesh with bone hierarchy and material properties.
- **Integration Points**: Render graph and shadow maps.
- **Performance Budget**: Character rig update $<0.05\text{ms}$ per character.
- **Acceptance Criteria**:
  - *Functional*: 3D human character meshes render with proper proportions and bone hierarchy.
  ...
  - *Documentation*: Bone naming convention documented.
- **Regression Tests**: Spawn 25 characters with varied clothing/skin types; inspect mesh integrity.
- **Deliverables**: `CharacterRig.js` and procedural human mesh renderer.
- **STOP Condition**: Pause for Phase Completion Gate verification after Phase 8 tests pass.

---

### PHASE 9 — Human Animation Architecture & IK
- **Objective**: Build animation blend trees (`NPCAnimation.js`) and inverse kinematics (`NPCIK.js`) for foot placement and hand object alignment.
- **Dependencies**:
  - *Depends on*: Phase 8.
  - *Blocks*: Phase 10, Phase 15.
- **Existing Systems Affected**: `src/NPC/NPCModel.js`, `src/Player/PlayerController.js`.
- **New Files**: `src/NPC/NPCAnimation.js`, `src/NPC/NPCIK.js`.
- **Modified Files**: `src/NPC/NPCModel.js`, `src/Player/PlayerController.js`.
- **Data Contracts & Serialization Schema**: Locomotion state `{ speed, turnRate, locomotionState, ikTargets }`.
- **Runtime Behavior**: Blends locomotion animations (Idle, Walk, Jog, Sprint, Cover) and aligns feet to terrain slopes and stairs via IK.
- **Integration Points**: Terrain collision raycasts.
- **Performance Budget**: IK solver $<0.08\text{ms}$ per active NEAR character.
- **Acceptance Criteria**:
  - *Functional*: Character feet snap naturally to sloped roads and stairs without floating or clipping.
  - *Integration*: Locomotion blends smoothly based on movement velocity.
  - *Performance*: 60 FPS maintained.
  - *Regression*: Player walking/running movement responsive.
  - *Stability*: 0 bone quaternion inversion errors.
  - *Documentation*: Animation state machine diagram created.
- **Regression Tests**: Walk character up stairs and sloped hills; observe foot IK placement.
- **Deliverables**: Locomotion blend tree engine and IK solver.
...
### PHASE 17 — Dynamic Outbreak World Director
- **Objective**: Build `OutbreakManager.js` controlling the 10 outbreak stages (Stage 0 Normal $\rightarrow$ Stage 9 City-Wide Collapse) modifying all world simulation parameters.
- **Dependencies**:
  - *Depends on*: Phase 12, Phase 13, Phase 16.
  - *Blocks*: Phase 18, Phase 20.
- **Existing Systems Affected**: `src/Core/WorldSimulation.js`, `src/Traffic/TrafficManager.js`, `src/Police/PoliceManager.js`.
- **New Files**: `src/Zombies/OutbreakManager.js`.
- **Modified Files**: `src/Core/WorldSimulation.js`.
- **Data Contracts & Serialization Schema**: Outbreak state `{ stage: 0..9, stageName, globalInfectionRate, districtStages: {} }`.
- **Runtime Behavior**: Dynamically escalates city condition: Stage 0 (Normal) $\rightarrow$ Stage 3 (Localized Outbreak) $\rightarrow$ Stage 6 (District Lockdown) $\rightarrow$ Stage 9 (Collapse), altering traffic density, police response, radio broadcasts, and store availability.
- **Integration Points**: All major world subsystems via `OUTBREAK_STAGE_CHANGED` events.
...
- **Acceptance Criteria**:
  ...
  - *Integration*: Power grid blackouts and metro disruptions accelerate local district outbreak stage.
  - *Performance*: 60 FPS target maintained.
...
- **Regression Tests**: Advance outbreak stage from Stage 0 to Stage 6; verify emergency alerts, barricades, and panic behaviors.
...
### PHASE 18 — Story Engine, Chapters & Branching Dialogue
- **Objective**: Build `StoryManager.js`, `ChapterManager.js`, and `DialogueSystem.js` for narrative chapter progression, dynamic subtitles, and branching dialogue choices.
- **Dependencies**:
  - *Depends on*: Phase 11, Phase 17.
  - *Blocks*: Phase 19, Phase 23.
- **Existing Systems Affected**: `src/UI/HUDManager.js`.
- **New Files**: `src/Story/StoryManager.js`, `src/Story/ChapterManager.js`, `src/Story/DialogueSystem.js`.
...
- **Runtime Behavior**: Drives Act progression (Prologue through Act VII), triggers dynamic dialogue overlays, and tracks branching narrative choices.
- **Integration Points**: NPC relationship matrix and interaction prompts.
- **Performance Budget**: Story tick $<0.05\text{ms}$.
- **Acceptance Criteria**:
  - *Functional*: Narrative progression executes chapters with active subtitles and branching options.
  - *Integration*: Dialogue choices react to past NPC relationship scores.
  - *Performance*: 60 FPS maintained.
...
- **Regression Tests**: Trigger Prologue dialogue; choose branching response; verify story flag update.
- **Deliverables**: Story chapter manager and branching dialogue tree.
...
- **Integration Points**: All major world, entity, and story subsystems.
